...
def main():
    ...
    ap.add_argument("--seq", default="core", choices=["core", "long"])
    ...
    t0 = time.time()
    for run in runs:
        r = run_one(run, seq, cfg, args.seed)
        r["seq"] = seq
    ...
    with open(args.out, "w") as f:
        json.dump({"config": cfg, "sequence": seq, "results": results}, f, ensure_ascii=False, indent=2)
