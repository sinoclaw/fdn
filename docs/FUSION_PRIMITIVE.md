...
## 7. 方向B（BDH式共享稀疏空间 + rho 层间传递）结果：CPU 上失败（2026-09-07 实测）

承接「加深版融合原语」的待定方向 B——把"每层独立全投影"换成**共享骨架（enc/dec/out 一套，参数不随深度增长）+ rho 状态层间 Hebbian 累积 + topk gather 只碰 k 激活通道**。
...
| 能力不输 | val ≤ TF+容差 | 3.42-3.45（差 0.47+） | ❌ 远差 TF/FusedDeep |
| 训练成本不线性 | 深度↑次线性 | 42.9→78.7→147.2s 近线性 | ❌ 线性且比 TF/FusedDeep 慢 |
| 推理降本 | wall-clock ≤ TF | 17→25→41ms 均慢于 TF | ❌ 随深度变慢 |

**结论**：方向B「共享稀疏空间」只换来**参数恒定**，换不来**每 pass 廉价**——`scatter_add`/`gather`/`einsum` 索引操作在 CPU torch 不是稀疏优化而是稠密开销，深度仍线性烧钱；且 rho 层间传递靠加深**不能涌现能力**（val 几乎不随 pass 动）。与 BDH/方向A加深一样落在"CPU 稀疏假红利"墙上。**方向B 不值得继续。** 详见 `results/EXPERIMENT_LOG.md` 2026-09-07 段。
